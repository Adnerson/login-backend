...
Future<Response> _getLoginStatus(RequestContext context) async {
  ...
  final redisKey = 'loggedin:$userId';
  final value = await context.read<Command>().send_object(['GET', redisKey]);

  if (value == null) {
    const status = 0;
    await context.read<Command>().send_object(['SET', redisKey, status]);
